...
namespace Tanks.SinglePlayer
{
	...
	public class Npc : MonoBehaviour, IDamageObject
	{
		...
		// <VENZELL> тут будем хранить все модификаторы уровня сложности, полученные из GameSettings
		[HideInInspector]
		public DifficultyLevelDetails npsDifficultyDetails;
		[SerializeField]
		...
		void Start()
		{
			// <VENZELL> Чем сложнее, тем больше здоровья у врагов и меньше у дружественных NPC
			if (isFriend) 
			{
				m_MaximumHealth /= npsDifficultyDetails.hpModifier;
			}
			else
			{
				m_MaximumHealth *= npsDifficultyDetails.hpModifier;
