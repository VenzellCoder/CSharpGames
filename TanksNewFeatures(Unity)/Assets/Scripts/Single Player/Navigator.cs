...
namespace Tanks.Rules.SinglePlayer
{
	...
	public class Navigator : MonoBehaviour
	{
		...
		private Tanks.SinglePlayer.DifficultyLevelDetails npsDifficultyDetails; 

		private void Awake()
		{
			m_MovingRigidBody = GetComponent<Rigidbody>();

			// <VENZELL> уровень сложности влияет на скорострельность
			npsDifficultyDetails = GetComponent<Tanks.SinglePlayer.Npc> ().npsDifficultyDetails;
			m_MoveSpeed *= npsDifficultyDetails.moveSpeedModifier;
		}
		...
		{
			if (c.collider.gameObject.layer == LayerMask.NameToLayer ("Players")) 
			{
				TankControllers.IDamageObject targetHealth = c.gameObject.GetComponentInParent<TankControllers.IDamageObject>();

				Vector3 vectorToenemy = transform.position - c.collider.transform.position;
				float angleToEnemy = Vector3.Angle (transform.forward, vectorToenemy);

				if (m_State == NavigationState.Moving && (angleToEnemy < 180 + 45 && angleToEnemy > 180 - 45)) 
				{
					if (targetHealth != null && targetHealth.isAlive) 
					{
						//Explosions.CrashManager.s_Instance.CmdCrash (c.collider.gameObject);
