...
namespace Tanks.Data
{
	...
	public class PlayerDataManager : Singleton<PlayerDataManager>
	{
		...
		// <VENZELL> Уровень сложности в одиночном режиме
		public int difficultyLevel
		{
			get
			{
				return m_Data.difficultyLevel;
			}
		}
		...
		// <VENZELL> Изменение уровня сложности
		public void ChangeDifficyltyLevel(int levelChange)
		{
			int difficultyLevelMin = 0;
			int difficultyLevelMax = GameSettings.s_Instance.m_DifficultyLevelList.Count;

			if (levelChange > 0 && m_Data.difficultyLevel < difficultyLevelMax) 
			{
				m_Data.difficultyLevel += levelChange;
				Debug.Log ("Повышение уровня! Now level = " + m_Data.difficultyLevel);
			}
			else if (levelChange < 0 && m_Data.difficultyLevel > difficultyLevelMin) 
			{
				m_Data.difficultyLevel += levelChange;
				Debug.Log ("Понижение уровня! Now level = " + m_Data.difficultyLevel);
			} 
